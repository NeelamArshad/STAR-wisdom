...
x %>%
  ...
  labs(title = "Histogram of 4th Grade Math Scores",
       x = "G4 Math Score",
       y = "Count") +
  theme_minimal()
```
...
x %>%
  ...
  labs(title = "Density Plot of 4th Grade Math Scores",
       x = "G4 Math Score",
       y = "Density") +
  theme_minimal()
```
...
Question: Does small number of students affect the performance than the large number of students in a Tennessee class?
This is a causal model.
Units are indvidual students. Outcomes are math scores. Covariates is race. Treatment is assignment to small class vs. large class.
...
preceptor_long <- x %>%
  mutate(
    ID = row_number(),
    Treatment = if_else(kind == "small", "Small", "Big")
  ) %>%
  select(ID, Outcome = g4math, Covariate = race, Treatment)
datatable(preceptor_long,
          caption = "Preceptor Table: Student-Level Outcomes, Covariates, and Treatment Group",
...
The population table is unit/time combination.

### Justice

Validity: Might not hold if the g4math test as four years ago is not the same test as today.

Stability: Might not be true if the relationship between the class size and math score from four years ago is not the same relationship as today.

Representativeness: Might not hold if the students are from different demographics.

Unconfoundedness: Might not hold if students assignment to small class vs. large class is not random. 
